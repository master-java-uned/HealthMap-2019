...
import uned.master.java.entity.Jwt;
import uned.master.java.entity.LoginUsuario;
...
public class AuthController {
	...
	/**
	 * Comprueba que el usuario y la contraseña coincidan con los datos de la base
	 * de datos
	 * 
	 * @param loginUsuario
	 * @param bindingResult
	...
	public ResponseEntity<Jwt> login(@Valid @RequestBody LoginUsuario loginUsuario, BindingResult bindingResult) {
		if (bindingResult.hasErrors())
			return new ResponseEntity("campos vacíos", HttpStatus.BAD_REQUEST);
		...
		Jwt jwtDTO = new Jwt(jwt, idUsuario, userDetails.getUsername(), userDetails.getAuthorities());
